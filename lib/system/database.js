...
global.loadDatabase = function loadDatabase() {
  ...
  const characters = conn.prepare(`SELECT clave, valor FROM characters`).all()
  for (const row of characters) {
    try { global.db.data.characters[row.clave] = JSON.parse(row.valor) } catch {}
  }

  const stickerspack = conn.prepare(`SELECT clave, valor FROM stickerspack`).all()
  for (const row of stickerspack) {
  ...
  global.db._snapshot.characters = JSON.stringify(global.db.data.characters)
  global.db._snapshot.stickerspack = JSON.stringify(global.db.data.stickerspack)
...
global.saveDatabase = function saveDatabase() {
  ...
  const insertCharacter = conn.prepare(`REPLACE INTO characters (clave, valor) VALUES (?, ?)`)
  for (const [clave, valor] of Object.entries(characters)) {
    insertCharacter.run(clave, JSON.stringify(valor))
  }

  const insertStickersPack = conn.prepare(`REPLACE INTO stickerspack (clave, valor) VALUES (?, ?)`)
  for (const [clave, valor] of Object.entries(stickerspack)) {
    insertStickersPack.run(clave, JSON.stringify(valor))
  }
  ...
  global.db._snapshot.characters = JSON.stringify(characters)
  global.db._snapshot.stickerspack = JSON.stringify(stickerspack)
}
